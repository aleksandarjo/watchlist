...
const Movie = ({ isTrailer = false, ytKey, ...movie }) => {
  return (
    <article className="group relative h-80 w-full overflow-hidden rounded-xl">
      ...
      <div
        className={`absolute inset-0  h-full w-full bg-black/15 ${!isTrailer ? "opacity-0" : "opacity-100"} transition-all group-hover:bg-black/50 group-hover:opacity-100`}
      >
      ...
      <div
        ...
        style={{
          backgroundImage: `${isTrailer ? `url('https://img.youtube.com/vi/${ytKey}/mqdefault.jpg')` : movie.backdrop_path ? `url(https://image.tmdb.org/t/p/original/${movie.backdrop_path})` : `url(${noImage})`}`,
